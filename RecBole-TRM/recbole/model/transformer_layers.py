...
class TimeAwareMultiHeadAttention_PIB(nn.Module):
    """
    Multi-head Self-attention layers, a attention score dropout layer is introduced.
    ...
    def __init__(self, n_heads, hidden_size, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps):
        super(TimeAwareMultiHeadAttention_PIB, self).__init__()
        if hidden_size % n_heads != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple of the number of attention "
                "heads (%d)" % (hidden_size, n_heads)
        ...
        # My Modification
        self.R = nn.Parameter(torch.zeros(1, 50), requires_grad=True)
        self.leaky_relu = nn.LeakyReLU(0.2)
        self.register_buffer('upper_triangular', torch.triu(torch.ones(50, 50)))
        trans_matrix = torch.zeros((50, 50, 50))
        for i in range(50):
            trans_matrix[i] = torch.diag(torch.ones(i+1), diagonal=50-i-1)
    ...
    def forward(self, input_tensor, attention_mask, absolute_pos_K, absolute_pos_V, time_matrix_emb_K, time_matrix_emb_V):
        ...
        # My Modification
        R_activated = self.leaky_relu(self.R)
        R_scaled = torch.matmul(R_activated, self.upper_triangular) 
        R_scaled_trans = torch.einsum('ijk,hk->hij', self.trans_matrix, R_scaled)
        R_scaled_trans = R_scaled_trans.unsqueeze(0)
        attention_scores = attention_scores + R_scaled_trans
...
class TimeAwareTransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        if PIB:
            self.multi_head_attention = TimeAwareMultiHeadAttention_PIB(
...
class TimeAwareTransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        layer = TimeAwareTransformerLayer(
            n_heads, hidden_size, inner_size, hidden_dropout_prob, attn_dropout_prob, hidden_act, layer_norm_eps, PIB
        )
